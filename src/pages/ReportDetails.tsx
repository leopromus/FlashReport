...
const ReportDetails = () => {
  ...
  const handleUpdate = async (values: Partial<typeof report>) => {
    if (id) {
      try {
        const updateData: Partial<Report> = {};
        
        // Only include fields that have been changed
        if (values.location !== report.location) {
          updateData.location = values.location;
        }
        if (values.comment !== report.comment) {
          updateData.comment = values.comment;
        }
        if (values.status !== report.status && user?.role === 'ADMIN') {
          updateData.status = values.status;
        }
        if (values.title !== report.title) {
          updateData.title = values.title;
        }
        if (values.type !== report.type) {
